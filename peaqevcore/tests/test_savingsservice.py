...
PRICES1 = [0.61, 0.58, 0.57, 0.57, 0.62, 0.65, 0.85, 1.02, 1.05, 1.04, 1.03, 0.89, 0.84, 0.78, 0.73, 0.73, 0.76, 0.89, 0.92, 0.9, 0.85, 0.72, 0.63, 0.51]
PRICES2= [0.028, 0.019, 0.001, 0.001, 0.001, 0.001, 0.001, 0.007, 0.028, 0.043, 0.062, 0.084, 0.084, 0.079, 0.246, 0.335, 0.508, 0.64, 0.754, 0.745, 0.668, 0.621, 0.639, 0.486]
...
async def test_calculate_charge_cost():
    ...
    _mock_charge_draw = {_connect_car_at.date(): {21: 1.2, 22: 1.2, 23: 1.6}}
    _assumed_sum = sum([v*PRICES1[k] for k,v in _mock_charge_draw[_connect_car_at.date()].items()])
...
async def test_wait_charge_next_day_threephase():
    ...
    _connect_car_at = datetime(2023,4,20,12,00,0)
    await s.async_add_prices(_date=_connect_car_at.date(), prices=_prices)
    await s.async_add_prices(_date=(_connect_car_at+timedelta(days=1)).date(), prices=_prices_tomorrow)
    await s.async_start_listen(_connect_car_at)
    _date = _connect_car_at.date()
    _hour = _connect_car_at.hour
    #register the draw before charging begins
