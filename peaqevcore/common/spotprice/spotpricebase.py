...
from peaqevcore.common.spotprice.spotprice_average_mixin import SpotPriceAverageMixin
...
class SpotPriceBase(SpotPriceAverageMixin):
    def __init__(self, hub, source: str, observer = MockObserver(), test:bool = False, is_active: bool = True):
    ...
    def purchased_average_month(self) -> float:
        try:
            month_draw = self.hub.state_machine.states.get("sensor.peaqev_energy_including_car_monthly") #todo: fix this
            month_cost = self.hub.state_machine.states.get("sensor.peaqev_energy_cost_integral_monthly") #todo: fix this
            if month_cost and month_draw:
                try:
                    return round(float(month_cost.state) / float(month_draw.state),3)
                except ValueError as v:
                    _LOGGER.warning(f"Unable to calculate purchased_average_month. {v}")
            return 0
