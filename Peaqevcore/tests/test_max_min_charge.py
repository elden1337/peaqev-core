...
class Helpers:
    ...
    async def setup_init_today_tomorrow():        
        ...
        h = MaxMinCharge()
        await h.async_make_hours(non_hours, dynamic_caution_hours, prices, prices_tomorrow, mock_hour = 21)
        return h    
    
    @staticmethod
    async def setup_init_today():        
        non_hours = [9, 10, 11, 12, 13, 14, 15, 16, 20, 21]
        prices = [0.54, 0.55, 0.57, 0.6, 0.62, 0.72, 0.74, 0.88, 0.9, 0.83, 0.77, 0.81, 0.77, 0.72, 0.61, 0.55, 0.51, 0.56, 0.58, 0.52, 0.44, 0.42, 0.34, 0.15]
        prices_tomorrow = []
        dynamic_caution_hours = {}
        h = MaxMinCharge()
        await h.async_make_hours(non_hours, dynamic_caution_hours, prices, prices_tomorrow, mock_hour = 9)
        return h    
...
async def test_lower_today(helpers):
    avg = 0.69
    peak = 2.28
    h = await helpers.setup_init_today()
    _non = h.non_hours[:]
    await h.async_update(avg, peak, 5)
    assert h.total_charge == 17.4
    assert len(h.non_hours) == len(_non)
