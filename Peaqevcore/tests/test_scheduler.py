...
def test_scheduler_overnight_1():
    s = Scheduler()
    MOCKDT = datetime.strptime('22-07-18 22:00', '%y-%m-%d %H:%M')
    dep_time_obj = datetime.strptime('22-07-19 06:00', '%y-%m-%d %H:%M')
    s.create(desired_charge=11, departuretime=dep_time_obj)
    s.update(avg24=710, peak=4.71, charged_amount=0, prices=MOCKPRICES, prices_tomorrow=MOCKPRICES_TOMORROW,mockdt=MOCKDT)
    assert s.model.hours_charge == {datetime(2022, 7, 19, 3, 0): 1, datetime(2022, 7, 19, 4, 0): 1, datetime(2022, 7, 19, 1, 0): 0.7}